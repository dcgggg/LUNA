...
def plot_waveforms(
    ...
    region_values = regions if regions is not None else ["未映射"] * len(ch_names)
    color_map = channel_colors([{"channel_name": name, "region": region} for name, region in zip(ch_names, region_values, strict=False)], DEFAULT_COLOR_TEMPLATE)
    for channel_index, (axis, name) in enumerate(zip(axes, ch_names)):
        for epoch_index in range(n_epochs):
            axis.plot(times, data[epoch_index, channel_index, :n_samples], linewidth=0.6, alpha=0.6, color=color_map.get(name, "#777777"))
    ...
    fig.subplots_adjust(left=0.16, right=0.97, bottom=0.08, top=0.94, hspace=0.18)
...
def plot_quality_matrix(quality: pd.DataFrame, output_base: str | Path, dpi: int = 150) -> tuple[Path, Path]:
    ...
    fig.subplots_adjust(left=0.12, right=0.88, bottom=0.18, top=0.88)
...
def plot_psd(psd_summary: pd.DataFrame, output_base: str | Path, dpi: int = 150, title: str = "Channel PSD") -> tuple[Path, Path]:
    ...
    if psd_summary.empty:
        ...
        metadata = psd_summary[[column for column in ("channel_name", "region") if column in psd_summary.columns]].drop_duplicates().to_dict("records")
        ...
        for channel_name, group in psd_summary.groupby("channel_name", sort=False):
            axis.plot(group["frequency_hz"], group["psd_value"], linewidth=0.8, color=color_map.get(str(channel_name), "#777777"), label=channel_name)
        axis.set_yscale("log")
...
def plot_band_power(band_power: pd.DataFrame, output_base: str | Path, dpi: int = 150) -> tuple[Path, Path]:
    ...
    fig.subplots_adjust(left=0.12, right=0.97, bottom=0.12, top=0.90, hspace=0.35)
    return _save(fig, output_base, dpi=dpi)
...
def plot_parameterization_fit(
    ...
    fig.subplots_adjust(left=0.08, right=0.97, bottom=0.10, top=0.90, wspace=0.30, hspace=0.55)
    return _save(fig, output_base, dpi=dpi)
...
def plot_parameterization_components(
    ...
    fig.subplots_adjust(left=0.08, right=0.97, bottom=0.12, top=0.90, wspace=0.30, hspace=0.55)
    return _save(fig, output_base, dpi=dpi)
...
def plot_connectivity_redundancy(
    ...
    fig.subplots_adjust(left=0.08, right=0.96, bottom=0.12, top=0.90, wspace=0.30, hspace=0.45)
    return _save(fig, output_base, dpi=dpi)
